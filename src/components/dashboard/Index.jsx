...
<div>
    <div className='grid grid-cols-3 md:grid-cols-1 gap-5'>
        ...
        <div className='flex justify-center items-center p-5 bg-white rounded-md gap-5'>
            <div className='bg-[#e2e2e2] w-[47px] h-[47px] rounded-full flex justify-center items-center text-xl'>
        <span className='text-xl text-gray-600'><RiShoppingCart2Fill /></span>
            </div>
...
<table className='w-full text-sm text-left text-gray-500'>
    <thead className='text-xs text-gray-700 uppercase bg-[#e2e2e2]'>
        <tr>
        ...
        <tbody>
            {
                ...
                <td scope='row' className='px-6 py-4 font-medium whitespace-nowrap'>
                    <Link to={`/dashboard/order/details/${o._id}`}><span className='bg-[#e2e2e2] text-gray-600 text-md font-semibold mr-2 px-3 py-[2px] rounded'>View</span></Link>

                    {
                       o.payment_status !== 'paid' && <span onClick={() => redirect(o)} className='bg-[#e2e2e2] text-gray-600 text-md font-semibold mr-2 px-3 py-[2px] rounded cursor-pointer'>Pay Now</span> 
                    }
